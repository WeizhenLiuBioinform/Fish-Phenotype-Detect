...
def coco_remove_images_without_annotations(dataset, ids):
    """
    Remove images without targets or with very small targets from the COCO dataset
    refer to:
...
def convert_coco_poly_mask(segmentations, height, width):
    ...
    if masks:
        ...
        # If mask is empty, it means there are no targets, directly return a mask with value 0
